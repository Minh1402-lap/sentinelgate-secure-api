...
export function createUpdateProfileHandler(users: UserService): RequestHandler {
  return async (request, response) => {
    const user = await users.updateProfile(authenticatedUserId(request.auth), request.body);
    response.status(200).json({ success: true, data: { user } });
  };
}

export function createListUsersHandler(users: UserService): RequestHandler {
  return async (request, response) => {
    ...
    response.status(200).json({ success: true, data: await users.list(query.data.page, query.data.pageSize) });
  };
}

export function createGetUserHandler(users: UserService): RequestHandler {
  return async (request, response) => {
    ...
    response.status(200).json({ success: true, data: { user: await users.getById(params.data.id) } });
  };
}
